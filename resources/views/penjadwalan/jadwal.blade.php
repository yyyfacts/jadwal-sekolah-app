                ...
                <div
                    ...
                    <div
                        ...
                        <ul class="list-disc list-inside space-y-1">
                            @foreach($latestMetrics['detail_pelanggaran_soft'] ?? [] as $ps)
                            <li>{{ $ps }}</li>
                            @endforeach
                        </ul>
                    </div>
        ...
        <div class="px-6 py-4 bg-white shrink-0 z-20 border-b border-slate-100">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
                    ...
                    {{-- FORM GENERATE — input waktu bebas diisi user --}}
                    <form action="{{ route('jadwal.generate') }}" method="POST" onsubmit="showLoading()"
                        class="m-0 p-0 flex items-center gap-2">
                        @csrf
                        <div class="relative flex items-center">
                            <span class="absolute left-3 text-slate-400 text-xs pointer-events-none">⏱️</span>
                            <input type="number" name="max_time" min="1" max="60" value="5"
                                title="Batas Waktu AI (menit)"
                                class="w-[110px] pl-8 pr-8 py-2 bg-white border border-slate-300 rounded-xl text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-center appearance-none cursor-pointer"
                                placeholder="Menit">
                            <span
                                class="absolute right-3 text-slate-400 text-[10px] pointer-events-none font-bold">mnt</span>
                        </div>
                        <button type="button"
                            onclick="if(confirm('Peringatan: AI akan menyusun jadwal sesuai batas waktu yang diisi. Lanjut?')) this.form.submit()"
...
<div id="modal-cek-guru"
    class="hidden fixed inset-0 z-[99999] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-2 sm:p-4">
    <div
        ...
        <div class="flex flex-col flex-1 min-h-0 p-4 bg-white">
            <div
                ...
                <div class="sm:col-span-1">
                    ...
                    <select id="cg-hari"
                        class="w-full border border-slate-300 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-blue-500 outline-none bg-white">
...
<div id="loading-overlay"
    class="hidden fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-slate-900/70 backdrop-blur-sm">
    <div class="bg-white p-8 rounded-3xl shadow-2xl text-center">
        ...
        <p class="text-[11px] text-slate-500 font-mono mt-2 max-w-[220px] mx-auto">Proses berjalan sesuai batas waktu
            yang kamu isi. Harap bersabar.</p>
    </div>
</div>
@endpush
        ...
        new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: ctx => ' Penalti: ' + ctx.parsed.y
                        }
                    }
                }
            }
...
function filterJadwalRealtime() {
    ...
    cells.forEach(c => {
        const match = c.getAttribute('data-search') && c.getAttribute('data-search').includes(input);
        c.style.opacity = match ? '1' : '0.15';
        c.style.filter = match ? 'none' : 'grayscale(100%)';
    });
}
...
function prosesCekGuru() {
    ...
    document.querySelectorAll('.jadwal-row').forEach(row => {
        ...
        if (rMulai < tSelesai && rSelesai > tMulai) {
            row.querySelectorAll('.jadwal-cell').forEach(cell => {
    ...
    if (jumlahKosong > 0) {
        html += `<div class="col-span-full mb-1 mt-1 border-b border-emerald-100 pb-1">
            <span class="text-[10px] font-extrabold text-emerald-600 uppercase flex items-center gap-1">🟢 Daftar Guru Tersedia (Free)</span>
        </div>`;
        guruKosongMap.forEach((nama, kode) => {
            html += `<div class="bg-emerald-50/50 border border-emerald-200 shadow-sm p-2 rounded-lg flex flex-col justify-center hover:border-emerald-300 transition-colors">
                <span class="text-[11px] font-bold text-slate-800 break-words leading-tight">${nama}</span>
                <div class="mt-1 flex items-center gap-1">
                    <span class="px-1.5 py-0.5 bg-emerald-100 text-emerald-700 rounded text-[8px] font-bold uppercase tracking-wider">Kosong / Available</span>
                </div>
            </div>`;
        });
    }
    ...
    if (jumlahMengajar > 0) {
        html += `<div class="col-span-full mb-1 mt-4 border-b border-slate-200 pb-1">
            <span class="text-[10px] font-extrabold text-blue-600 uppercase flex items-center gap-1">🔵 Daftar Guru Sedang Mengajar</span>
        </div>`;
        guruMengajarMap.forEach((data, kode) => {
            const daftarKelas = Array.from(data.kelas).join(', ');
            html += `<div class="bg-white border border-slate-200 shadow-sm p-2 rounded-lg flex flex-col justify-center hover:border-blue-300 transition-colors">
