                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-4 py-2 text-right align-middle">
                            <div class="flex items-center justify-end gap-1.5">
                                {{-- Tombol EDIT Sekarang Selalu Muncul --}}
                                <a href="{{ route('user.edit', $u->id) }}" title="Edit Pengguna"
                                    class="p-1.5 border rounded-lg text-slate-400 hover:text-blue-600 hover:border-blue-300 transition-colors bg-white">
                                ...
                                {{-- Tombol HAPUS Tetap Dibatasi (Tidak Bisa Hapus Diri Sendiri) --}}
                                @if(Auth::id() != $u->id)
                                <form action="{{ route('user.destroy', $u->id) }}" method="POST" class="inline m-0"
                                    ...
                                    <button type="submit" title="Hapus Pengguna"
                                        class="p-1.5 border rounded-lg text-slate-400 hover:text-red-500 hover:border-red-300 transition-colors bg-white">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                @else
                                <span
                                    class="text-[9px] font-bold text-emerald-600 italic bg-emerald-50 border border-emerald-100 px-2 py-1 rounded ml-1">Akun
                                    Anda</span>
                                @endif
                            </div>
                        </td>
