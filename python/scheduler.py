...
def main():
    ...
    # max_jam_map sekarang murni berisi TOTAL JAM BELAJAR (misal 10), bukan jam fisik 11.
    ...
    for t in raw_assignments:
        ...
        for h in hari_list:
            ...
            # KEMBALI KE LOGIKA SIMPLE TANPA MIKIR ISTIRAHAT
        ...
        if possible_days:
            ...
            print(json.dumps({"status": "INFEASIBLE", "message": f"Mapel ID {t_id} kepanjangan."}))
            return
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        print(json.dumps({"status": "OPTIMAL", "solution": final_solution, "waktu_komputasi_detik": round(waktu_komputasi, 2), "message": f"Sukses dalam {waktu_komputasi:.2f} detik."}))
    else:
        print(json.dumps({"status": "INFEASIBLE", "waktu_komputasi_detik": round(waktu_komputasi, 2), "message": f"Gagal menyusun."}))
